...
function OtpBoxes({ value, onChange, disabled }) {
  ...
  const handleKey = (e, i) => {
    if (e.key === "Backspace") {
      ...
      onChange(next.join("").trimEnd()); return
    }
    if (/^\d$/.test(e.key)) {
  ...
  const handlePaste = (e) => {
    ...
    onChange(pasted); inputs.current[Math.min(pasted.length,5)]?.focus(); e.preventDefault()
  }
      ...
      {digits.map((d, i) => {
        ...
        return (
          <input key={i} ref={el=>(inputs.current[i]=el)}
            ...
            value={filled?d:""} onChange={() => {}}
            onKeyDown={e=>handleKey(e,i)} onPaste={handlePaste} disabled={disabled}
            style={{ width:"clamp(36px,10vw,48px)", height:"clamp(46px,11vw,56px)",
              textAlign:"center", fontSize:"clamp(18px,4vw,22px)",
              fontFamily:"'Cormorant Garamond', serif", fontWeight:700,
              background:filled?"rgba(201,169,110,0.18)":"rgba(255,255,255,0.06)",
              backdropFilter:"blur(4px)",
              border:`1px solid ${filled?"rgba(201,169,110,0.7)":"rgba(255,255,255,0.12)"}`,
              borderRadius:"6px", color:"#f5edd8", outline:"none",
              transition:"all 0.2s", opacity:disabled?0.4:1,
              caretColor:"#c9a96e", WebkitTextFillColor:"#f5edd8",
...
function PasswordField({ label, name, value, onChange, required }) {
  ...
  return (
    <Field label={label} name={name} type={show?"text":"password"} value={value} onChange={onChange} required={required} IconLeft={IconLock}
      rightSlot={
      ...
      {labels.map((lbl, i) => (
        <React.Fragment key={i}>
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"6px" }}>
            <div style={{ width:"clamp(28px,6vw,36px)", height:"clamp(28px,6vw,36px)", borderRadius:"50%",
              background:i<current?"linear-gradient(135deg,#c9a96e,#9e7035)":i===current?"rgba(201,169,110,0.12)":"rgba(255,255,255,0.05)",
              ...
              fontWeight:700, fontSize:"clamp(11px,1.5vw,13px)",
              fontFamily:"'Playfair Display', serif", transition:"all 0.4s",
              boxShadow:i===current?"0 0 0 4px rgba(201,169,110,0.12), 0 4px 12px rgba(201,169,110,0.15)":i<current?"0 4px 12px rgba(201,169,110,0.2)":"none" }}>
              {i < current ? (
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#160b05" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              ) : i + 1}
            </div>
            <span style={{ fontSize:"clamp(8px,1.2vw,9px)", letterSpacing:"0.16em", textTransform:"uppercase",
              color:i===current?"rgba(201,169,110,0.85)":"rgba(255,255,255,0.2)",
              fontFamily:"'Cormorant Garamond', serif", fontWeight:500 }}>{lbl}</span>
          </div>
          {i < labels.length - 1 && (
            <div style={{ width:"clamp(40px,12vw,80px)", height:"1px", margin:"0 clamp(6px,1.5vw,10px)",
              marginBottom:"clamp(16px,2.5vw,22px)",
              background:i<current?"linear-gradient(90deg,rgba(201,169,110,0.8),rgba(158,112,53,0.6))":"rgba(255,255,255,0.08)",
...
export default function Register() {
  ...
  const showToast = (msg, type="gold") => {
    setToast({ msg, type }); setTimeout(() => setToast({ msg:"", type:"gold" }), 3500)
  }
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .rp-card {
          ...
          padding:clamp(24px,5vw,50px) clamp(16px,5vw,48px) clamp(22px,4vw,44px);
          box-shadow:
        ...
        .rp-card::before { content:''; position:absolute; top:0; left:10%; right:10%; height:1px; background:linear-gradient(90deg,transparent,rgba(255,255,255,0.15),transparent); border-radius:50%; }

        .rp-frame   { position:absolute; inset:10px; border:1px solid rgba(201,169,110,0.08); border-radius:8px; pointer-events:none; }
        .rp-bracket { position:absolute; width:20px; height:20px; border-color:rgba(201,169,110,0.35); border-style:solid; }
        ...
        .rp-eyebrow { text-align:center; font-size:clamp(11px,1.5vw,14px); letter-spacing:0.3em; text-transform:uppercase; color:#f8f0e0; margin-bottom:8px; }
        .rp-title   { text-align:center; font-family:'Playfair Display', serif; font-size:clamp(22px,4vw,30px); font-weight:600; color:rgb(201,169,110); line-height:1.15; margin-bottom:clamp(20px,3vw,32px); text-shadow:0 2px 20px rgba(0,0,0,0.5); }
        ...
        .rp-btn {
          width:100%; margin-top:clamp(18px,3vw,26px); padding:clamp(13px,2vw,16px);
        ...
        .rp-btn::before { content:''; position:absolute; inset:0; background:linear-gradient(135deg,rgba(255,255,255,0.2) 0%,transparent 55%); pointer-events:none; }
        .rp-btn:hover:not(:disabled) { background-position:right center; box-shadow:0 8px 32px rgba(201,169,110,0.4),0 0 0 1px rgba(201,169,110,0.3); transform:translateY(-2px); }
        .rp-btn:active:not(:disabled) { transform:translateY(0); }
        .rp-btn:disabled { opacity:0.4; cursor:not-allowed; }

        .rp-btn-ghost {
          width:100%; margin-top:10px; padding:clamp(12px,1.5vw,14px);
          background:rgba(255,255,255,0.04); backdrop-filter:blur(4px);
          border:1px solid rgba(255,255,255,0.1); border-radius:8px;
          color:rgba(220,200,160,0.45); font-family:'Cormorant Garamond', serif;
          font-size:clamp(11px,1.5vw,12px); letter-spacing:0.25em; text-transform:uppercase;
        ...
        .rp-toast {
          ...
          font-family:'Cormorant Garamond', serif;
          font-size:clamp(12px,1.5vw,14px); letter-spacing:0.06em;
          border-radius:8px; white-space:nowrap; z-index:999;
          backdrop-filter:blur(20px); animation:toastIn 0.4s cubic-bezier(0.34,1.56,0.64,1);
        }
        .rp-toast.gold  { background:rgba(20,10,3,0.85); border:1px solid rgba(201,169,110,0.4); color:#d4b46a; box-shadow:0 8px 32px rgba(0,0,0,0.5),0 0 20px rgba(201,169,110,0.1); }
        .rp-toast.green { background:rgba(5,20,5,0.85); border:1px solid rgba(80,180,80,0.4); color:#80d080; box-shadow:0 8px 32px rgba(0,0,0,0.5); }
        @keyframes toastIn { from{opacity:0;transform:translateX(-50%) translateY(18px) scale(0.93)} to{opacity:1;transform:translateX(-50%) translateY(0) scale(1)} }

        .rp-otp-hint { text-align:center; color:rgba(220,200,160,0.45); font-size:clamp(12px,1.5vw,14px); line-height:1.8; margin-bottom:clamp(16px,2.5vw,28px); font-style:italic; }
        .rp-otp-hint strong { color:rgba(201,169,110,0.85); font-style:normal; font-weight:500; }

        .rp-resend { text-align:center; margin-top:clamp(12px,2vw,18px); font-size:clamp(12px,1.5vw,13px); color:rgba(201,169,110,0.28); letter-spacing:0.04em; }
        .rp-resend-btn { background:none; border:none; padding:0; font-family:'Cormorant Garamond', serif; font-size:clamp(12px,1.5vw,13px); color:rgba(201,169,110,0.6); cursor:pointer; border-bottom:1px solid rgba(201,169,110,0.25); transition:all 0.2s; letter-spacing:0.04em; }
        ...
        .rp-verified { display:inline-flex; align-items:center; gap:7px; background:rgba(40,100,40,0.15); backdrop-filter:blur(6px); border:1px solid rgba(80,160,80,0.25); color:#78c878; padding:7px 16px; border-radius:100px; font-size:clamp(11px,1.5vw,12px); letter-spacing:0.08em; margin-bottom:clamp(12px,2vw,18px); }

        .rp-bottom { text-align:center; margin-top:clamp(14px,2.5vw,22px); font-size:clamp(13px,1.5vw,15px); color:rgba(221,217,217,0.91); letter-spacing:0.06em; }
        .rp-link   { color:rgba(242,218,174,0.6); text-decoration:underline; transition:color 0.2s; }
        .rp-link:hover { color:#c9a96e; }

        .rp-divider { display:flex; align-items:center; gap:14px; color:rgba(255,255,255,0.1); font-size:10px; letter-spacing:0.4em; text-transform:uppercase; margin-top:clamp(16px,2.5vw,22px); }
        .rp-divider::before, .rp-divider::after { content:''; flex:1; height:1px; background:rgba(255,255,255,0.07); }

        .rp-spinner { display:inline-block; width:13px; height:13px; border:2px solid rgba(22,11,5,0.3); border-top-color:rgba(22,11,5,0.8); border-radius:50%; animation:spin 0.7s linear infinite; margin-right:8px; vertical-align:middle; }
      ...
      <div className="rp-root">
        <div className="rp-card fade-up">
          ...
          <h1 className="rp-title">{step===0?"Create Account":"Verify Your Email"}</h1>
          ...
          {step === 0 && (
            <form onSubmit={handleSendOtp} className="fade-up">
              ...
              <p className="rp-bottom">
                Already a member?&nbsp;<Link to="/login" className="rp-link">Sign In</Link>
              </p>
            </form>
          )}
