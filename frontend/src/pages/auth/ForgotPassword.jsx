...
function SlideshowBg({ images }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 0, overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", inset: 0, zIndex: 1, background: `linear-gradient(to bottom, rgba(5,2,1,0.45) 0%, rgba(8,4,2,0.55) 40%, rgba(5,2,1,0.75) 100%)` }} />
      <div style={{ position: "absolute", inset: 0, zIndex: 2, background: `radial-gradient(ellipse at center, transparent 40%, rgba(0,0,0,0.55) 100%)` }} />
      <div style={{ position: "absolute", bottom: "28px", left: "50%", transform: "translateX(-50%)", display: "flex", gap: "8px", zIndex: 10 }}>
        {images.map((_, i) => (
          <div key={i} style={{ width: i === current ? "24px" : "6px", height: "6px", borderRadius: "3px", background: i === current ? "rgba(201,169,110,0.8)" : "rgba(201,169,110,0.25)", transition: "all 0.5s cubic-bezier(0.4,0,0.2,1)" }} />
        ))}
      </div>
      <style>{`
...
function Field({ label, name, type = "text", value, onChange, required, IconLeft, rightSlot, disabled }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      <span style={{ position: "absolute", left: "14px", top: "50%", transform: "translateY(-50%)", opacity: active ? 1 : 0.5, display: "flex", alignItems: "center", pointerEvents: "none", zIndex: 2, transition: "opacity 0.2s" }}>
        <IconLeft color={focused ? "#e8c980" : "rgba(201,169,110,0.7)"} />
      </span>
      <label style={{ position: "absolute", left: "44px", top: active ? "7px" : "50%", transform: active ? "none" : "translateY(-50%)", fontSize: active ? "9px" : "13.5px", color: focused ? "#e8c980" : active ? "rgba(201,169,110,0.8)" : "rgba(220,200,160,0.4)", transition: "all 0.22s cubic-bezier(0.4,0,0.2,1)", pointerEvents: "none", letterSpacing: active ? "0.14em" : "0.06em", textTransform: "uppercase", fontFamily: "'Cormorant Garamond', serif", fontWeight: 500, zIndex: 2 }}>
        {label}
      </label>
      <input
        ...
        style={{ width: "100%", height: "56px", paddingTop: "20px", paddingBottom: "6px", paddingLeft: "44px", paddingRight: rightSlot ? "46px" : "14px", background: disabled ? "rgba(255,255,255,0.02)" : focused ? "rgba(201,169,110,0.1)" : "rgba(255,255,255,0.06)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", border: `1px solid ${focused ? "rgba(201,169,110,0.55)" : "rgba(255,255,255,0.1)"}`, borderRadius: "8px", color: disabled ? "rgba(245,237,216,0.4)" : "#f5edd8", fontSize: "15px", fontFamily: "'Cormorant Garamond', serif", outline: "none", transition: "all 0.22s", boxSizing: "border-box", WebkitTextFillColor: disabled ? "rgba(245,237,216,0.4)" : "#f5edd8", WebkitBoxShadow: "0 0 0px 1000px rgba(10,5,2,0.01) inset", caretColor: "#c9a96e", boxShadow: focused ? "0 0 0 3px rgba(201,169,110,0.08), inset 0 1px 0 rgba(255,255,255,0.06)" : "inset 0 1px 0 rgba(255,255,255,0.04)" }}
      />
      {rightSlot && (
        <span style={{ position: "absolute", right: "14px", top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", cursor: "pointer", zIndex: 2 }}>
          {rightSlot}
        </span>
      )}
    </div>
...
function PasswordField({ label = "New Password", name = "password", value, onChange }) {
  ...
  return (
    <Field label={label} name={name} type={show ? "text" : "password"} value={value} onChange={onChange} required IconLeft={IconLock}
      rightSlot={
        <span onClick={() => setShow(s => !s)} style={{ display: "flex", opacity: 0.5, transition: "opacity 0.2s" }} onMouseEnter={e => e.currentTarget.style.opacity = 1} onMouseLeave={e => e.currentTarget.style.opacity = 0.5}>
      ...
      {digits.map((d, i) => (
        <input
          ...
          style={{
            ...
            caretColor: "#c9a96e",
            transition: "all 0.2s",
            boxShadow: d ? "0 0 0 3px rgba(201,169,110,0.08)" : "none",
            letterSpacing: "0",
          }}
        />
      ))}
          ...
          <React.Fragment key={i}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px" }}>
              <div style={{
                ...
                transition: "all 0.4s",
                boxShadow: active ? "0 0 14px rgba(201,169,110,0.3)" : "none",
              }}>
                {done ? (
                  ...
                  <span style={{ fontSize: "11px", color: active ? "#c9a96e" : "rgba(255,255,255,0.3)", fontFamily: "'Cormorant Garamond', serif", fontWeight: 600 }}>{i + 1}</span>
                )}
              </div>
              <span style={{ fontSize: "9px", letterSpacing: "0.12em", textTransform: "uppercase", color: done || active ? "rgba(201,169,110,0.8)" : "rgba(255,255,255,0.2)", fontFamily: "'Cormorant Garamond', serif", whiteSpace: "nowrap" }}>{label}</span>
            </div>
            {i < steps.length - 1 && (
              <div style={{ flex: 1, height: "1px", background: i < step ? "rgba(201,169,110,0.4)" : "rgba(255,255,255,0.08)", margin: "0 6px", marginBottom: "18px", transition: "background 0.4s", minWidth: "28px" }} />
            )}
          </React.Fragment>
        )
      })}
...
function ResendTimer({ onResend }) {
  ...
  return (
    <p style={{ textAlign: "center", fontSize: "13px", color: "rgba(220,200,160,0.45)", letterSpacing: "0.04em", fontFamily: "'Cormorant Garamond', serif" }}>
      {seconds > 0
        ? <>Resend OTP in <span style={{ color: "rgba(201,169,110,0.7)" }}>{seconds}s</span></>
        : (
          <button onClick={handleResend} disabled={loading} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", color: "rgba(242,218,174,0.65)", textDecoration: "underline", fontSize: "13px", fontFamily: "'Cormorant Garamond', serif", transition: "color 0.2s" }}
  ...
  return (
    <div style={{ marginTop: "6px" }}>
      <div style={{ display: "flex", gap: "4px", marginBottom: "5px" }}>
        {[1,2,3,4].map(i => (
          <div key={i} style={{ flex: 1, height: "3px", borderRadius: "2px", background: i <= strength ? colors[strength] : "rgba(255,255,255,0.08)", transition: "all 0.3s" }} />
        ))}
      </div>
      <p style={{ fontSize: "10px", letterSpacing: "0.12em", textTransform: "uppercase", color: colors[strength], fontFamily: "'Cormorant Garamond', serif", textAlign: "right" }}>
        {labels[strength]}
      </p>
    </div>
  )
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .fp-success-msg {
          display: flex; align-items: flex-start; gap: 10px;
          background: rgba(50,120,70,0.15);
          backdrop-filter: blur(8px);
          border: 1px solid rgba(80,160,100,0.25);
          border-left: 3px solid rgba(80,180,100,0.6);
          color: #70c88a; padding: 11px 14px;
          font-size: 13px; border-radius: 6px;
          margin-bottom: 18px; line-height: 1.5;
          animation: fadeUp 0.2s ease;
        }
        ...
        .fp-otp-label {
          ...
          color: rgba(201,169,110,0.6); margin-bottom: 14px;
          font-family: 'Cormorant Garamond', serif;
        }
        ...
        @media (max-width: 540px) {
          .fp-card { padding: 38px 22px 34px; }
        }
      `}</style>
        ...
        <div className="fp-card fade-up">
          ...
          {step === 0 && (
            <form key={`step0-${animKey}`} className="fade-in" onSubmit={handleSendOtp}>
              <Field
                label="Registered Email" name="email" type="email"
          ...
          {step === 2 && (
            <form key={`step2-${animKey}`} className="fade-in" onSubmit={handleResetPassword}>
              ...
              <button type="submit" className="fp-btn" disabled={loading || !passwords.password || passwords.password !== passwords.confirm}>
