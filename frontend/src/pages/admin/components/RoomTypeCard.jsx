...
// Change the UI here → both pages update automatically.
// ─────────────────────────────────────────────
      ...
      {images.map((img, i) => (
        <img
          ...
          style={{
            ...
            objectFit: "cover", borderRadius: "12px", top: 0, left: 0,
    ...
    <div className={`card-p anim-up d${Math.min(index + 1, 5)}`}>
      ...
      <div className="flex justify-between items-start mb-3">
        ...
        <span className="font-bold text-gold text-lg">
          ₹{t.price_per_night?.toLocaleString()}
        </span>
      </div>
      ...
      {/* Description */}
      {t.description && (
        <p className="text-resort-dim text-xs leading-relaxed mb-3">
          {t.description}
        </p>
      )}
        ...
        >
          {onEdit && (
            <Button size="xs" variant="outline"
              onClick={() => onEdit(t)} className="flex-1">
              Edit
            </Button>
          )}
          {onDelete && (
