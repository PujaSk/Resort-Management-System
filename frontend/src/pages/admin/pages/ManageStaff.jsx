...
import { IconWarning, PlusIcon, PencilIcon, RightTickIcon } from "../../../components/ui/Icons"
...
function FieldLabel({ text, required }) {
  return (
    <label style={{
      ...
      fontSize: 10, fontWeight: 700, color: "#6B6054",
...
export default function ManageStaff() {
  ...
  const openCreate = () => {
    setForm(emptyForm); setErrors(emptyErrors); setModal("create")
  }
  const openEdit = (row) => {
    setForm({
  ...
  const validate = () => {
    ...
    if (!form.staff_name.trim()) {
      errs.staff_name = "Full name is required"; ok = false
    } else if (form.staff_name.trim().length < 2) {
      errs.staff_name = "Name must be at least 2 characters"; ok = false
    }
    if (!form.email.trim()) {
    ...
    if (!phone) {
      errs.phoneno = "Phone number is required"; ok = false
    } else if (!/^[6-9]\d{9}$/.test(phone)) {
      errs.phoneno = "Enter valid 10-digit Indian mobile (starts with 6–9)"; ok = false
    }
    ...
    if (!form.salary) {
      ...
      errs.salary = "Enter a valid salary amount"; ok = false
    } else if (Number(form.salary) < 5000) {
      errs.salary = "Salary must be at least ₹5,000"; ok = false
    }
    if (!form.birth_date) {
      errs.birth_date = "Birth date is required"; ok = false
    } else {
    ...
    if (!form.joining_date) {
      errs.joining_date = "Joining date is required"; ok = false
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (modal === "create") {
        ...
        show("Staff added — login credentials sent to their email!")
      } else {
        await updateStaff(modal._id, payload)
        show("Staff updated successfully")
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteStaff(id); show("Staff removed successfully"); loadStaff()
  ...
  const columns = [
    ...
    { key: "shift",       label: "Shift",  render: v => <Badge label={v} variant="info" size="sm"/> },
    {
      ...
      render: v => v
        ? <span style={{ fontWeight: 600, color: "#52C07A" }}>₹{Number(v).toLocaleString("en-IN")}</span>
        : "—"
    },
    {
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            ...
            <FInput value={form.staff_name} placeholder="Enter full name"
              hasError={!!errors.staff_name}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              ...
              <FInput value={form.email} placeholder="staff@resort.com" type="email"
            ...
            <div>
              ...
              <div style={{ position: "relative" }}>
                <FInput
                  value={form.phoneno} placeholder="9876543210"
