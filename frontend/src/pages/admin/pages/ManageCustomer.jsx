...
const RESP = `
  ...
  @media (min-width: 480px) { .mc-stats-grid { grid-template-columns: repeat(2, 1fr); } }
  @media (min-width: 768px) { .mc-stats-grid { grid-template-columns: repeat(3, 1fr); } }
  ...
  @media (min-width: 480px) { .mc-booking-stats { grid-template-columns: repeat(4, 1fr); } }
  ...
  @media (max-width: 360px) { .mc-profile-grid { grid-template-columns: 1fr; } }
`
...
export default function ManageCustomer() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      allBookings.forEach(b => {
        ...
        if (!custMap[id]) custMap[id] = { ...c, bookings: [] }
  ...
  const cols = [
    ...
    {
      key:"bookings-spent", label:"Bookings / Spent",
      render: (_, r) => {
    ...
    },
    {
      ...
      render: (_, r) => <Button size="xs" variant="outline" onClick={() => setSelected(r)}>View Details</Button>
    },
  ]
            ...
            <div className="space-y-4">
              ...
              <div style={{ padding:"14px 16px", borderRadius:10, background:"rgba(255,255,255,.02)", border:"1px solid rgba(255,255,255,.06)" }}>
                <div className="flex items-center gap-3 mb-3">
                  <div style={{ width:48, height:48, borderRadius:12, background:"rgba(201,168,76,.1)", border:"1px solid rgba(201,168,76,.2)",
                    display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, fontWeight:700, color:"#C9A84C", flexShrink:0 }}>
                    {(selected.name||"G")[0].toUpperCase()}
                  </div>
                  <div style={{ minWidth:0 }}>
                    <p className="font-semibold text-cream text-base truncate">{selected.name}</p>
                    <p className="text-xs text-resort-dim truncate">{selected.email}</p>
                    <div className="mt-1">
                      <Badge label={selected.isEmailVerified?"Verified":"Unverified"} variant={selected.isEmailVerified?"Available":"Maintenance"} size="sm"/>
                    </div>
                  </div>
                </div>
                {/* RESPONSIVE: mc-profile-grid */}
                <div className="mc-profile-grid">
                  {[
                    ...
                    <div key={l} style={{ padding:"8px 10px", borderRadius:6, background:"rgba(255,255,255,.02)", border:"1px solid rgba(255,255,255,.04)" }}>
                      ...
                      <p style={{ fontSize:12, color:"#F5ECD7", fontWeight:500, wordBreak:"break-word" }}>{v}</p>
                    </div>
                  ))}
                </div>
              </div>

              {/* Booking stats — RESPONSIVE: mc-booking-stats */}
              <div className="mc-booking-stats">
                {[
                ...
                <div>
                  ...
                  <div className="space-y-2" style={{ maxHeight:260, overflowY:"auto" }}>
                    {bs.map(b => {
                      ...
                      return (
                        <div key={b._id} style={{ padding:"10px 12px", borderRadius:8, background:"rgba(255,255,255,.02)", border:"1px solid rgba(255,255,255,.05)", display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
                          <div style={{ minWidth:0 }}>
                            <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                              <span style={{ fontSize:12, fontWeight:600, color:"#F5ECD7" }}>
                                {b.room?`Room #${b.room.room_number}`:b.roomType?.type_name||"—"}
                              </span>
                              <span style={{ fontSize:10, padding:"1px 6px", borderRadius:20, background:`${bColor}15`, color:bColor, border:`1px solid ${bColor}22`, whiteSpace:"nowrap" }}>{b.bookingStatus}</span>
                            </div>
