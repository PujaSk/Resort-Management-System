...
const fmtTime = d => d ? new Date(d).toLocaleTimeString("en-IN",  { ...IST, hour:"2-digit", minute:"2-digit", hour12:true }) : "—"
...
function InfoRow({ label, value, valueColor, mono }) {
  return (
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,.04)"}}>
      <span style={{fontSize:12,color:"#6B6054"}}>{label}</span>
      <span style={{fontSize:13,fontWeight:600,color:valueColor||"#F5ECD7",fontFamily:mono?"monospace":"inherit"}}>{value}</span>
    </div>
  )
}
      ...
      {payment.method==="upi" && (
        <div>
          ...
          <input value={payment.upiId}
            onChange={e=>{ const v=e.target.value; setPayment(p=>({...p,upiId:v})); setErrors(er=>({...er,upiId:""})) }}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div>
            ...
            <input value={payment.cardName}
              onChange={e=>{ const v=e.target.value; setPayment(p=>({...p,cardName:v})); setErrors(er=>({...er,cardName:""})) }}
...
function validatePayment(payment, setErrors) {
  ...
  if (m==="credit_card"||m==="debit_card") {
    ...
    if (payment.expiry.length<5)                        errs.expiry     = "Enter expiry (MM/YY)"
      ...
      else { const now=new Date(); if (expYY<now.getFullYear()||(expYY===now.getFullYear()&&expMM<now.getMonth()+1)) errs.expiry = "Card has expired" }
    }
  }
  setErrors(errs)
  ...
  return (
    <div style={{
      ...
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        padding:"28px 32px",maxWidth:460,width:"100%",boxShadow:"0 40px 100px rgba(0,0,0,.8)",
      }}>
...
export default function BookingActionDetail() {
  ...
  const [panel,     setPanel]     = useState(null) // "checkin" | "checkout" | null
  ...
  const ciBlockMsg = isHallBk
    ? today<ciDate
      ? `Hall check-in only allowed on the event day: ${fmtDate(bk.checkInDateTime)}`
      : "Today is not the event date — check-in window has passed."
    : today<ciDate
      ? `Check-in not allowed yet — earliest: ${fmtDate(bk.checkInDateTime)}`
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      setPanel(null); setCiPayment({...EMPTY_PAY}); setCiErrors({}); setCiChoice("checkout")
      await load()
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      setPanel(null); setCoPayment({...EMPTY_PAY}); setCoErrors({})
      await load()
  ...
  return (
    <div style={{maxWidth:960,margin:"0 auto",padding:"0 0 80px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:20}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:20}}>
          ...
          <Card>
            ...
            <InfoRow label="Status" value={bk.bookingStatus}
              valueColor={STATUS_CLR[bk.bookingStatus]||"#6B6054"}/>

            {/* Actual check-in/out timestamps */}
            {bk.actualCheckInTime && (
              <InfoRow label="✓ Checked-In At"  value={fmtDT(bk.actualCheckInTime)} valueColor="#52C07A"/>
            )}
            {bk.actualCheckOutDate && (
              <InfoRow label="✓ Checked-Out At" value={fmtDT(bk.actualCheckOutDate)} valueColor="#52C07A"/>
            )}
            {bk.noShowMarkedAt && (
              <InfoRow label="⚠ No-Show At" value={fmtDT(bk.noShowMarkedAt)} valueColor="#E0A852"/>
            )}
          ...
          <Card>
            ...
            {bk.checkInPayment && (
              <div style={{marginTop:12}}>
                <p style={{fontSize:10,fontWeight:700,color:"#6B6054",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>At Check-In</p>
                <PayRec record={bk.checkInPayment} label="Check-In Payment"/>
              </div>
            )}
            {bk.checkOutPayment && (
              <div style={{marginTop:12}}>
                <p style={{fontSize:10,fontWeight:700,color:"#6B6054",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>At Check-Out</p>
                <PayRec record={bk.checkOutPayment} label="Check-Out Payment"/>
              </div>
            )}
            ...
            {bk.extendedStay?.isExtended && (
              <div style={{marginTop:14,padding:"10px 14px",borderRadius:10,background:"rgba(224,168,82,.06)",border:"1px solid rgba(224,168,82,.2)"}}>
                <p style={{fontSize:11,fontWeight:700,color:"#E0A852",marginBottom:6,display:"flex",alignItems:"center",gap:5}}>
                  <IconWarning size={12} color="#E0A852"/> Extended Stay Charges
                </p>
            ...
            {/* Early checkout summary */}
            {bk.earlyCheckout?.isEarly && (
              <div style={{marginTop:14,padding:"10px 14px",borderRadius:10,background:"rgba(224,168,82,.06)",border:"1px solid rgba(224,168,82,.2)"}}>
                <p style={{fontSize:11,fontWeight:700,color:"#E0A852",marginBottom:6,display:"flex",alignItems:"center",gap:5}}>
                  <IconWarning size={12} color="#E0A852"/> Early Checkout Summary
                </p>
      ...
      {/* ACTION SECTION — admin/staff only */}
      {isAdmin && !isDone && (
        <Card>
          ...
          {isBooked&&!panel&&canCheckIn && (
            <p style={{fontSize:12,color:"#8A7E6A",marginTop:0,marginBottom:0}}>
              Click <strong style={{color:"#C9A84C"}}>Check In</strong> to proceed.
            </p>
          )}
              ...
              <div style={{padding:18}}>
                ...
                {due>0 && (
                  <div style={{marginBottom:16}}>
                    ...
                    <div style={{display:"flex",gap:0,borderRadius:10,overflow:"hidden",border:"1px solid rgba(255,255,255,.08)"}}>
                      {[
                        ...
                        <button key={opt.val} type="button" onClick={() => {setCiChoice(opt.val);setCiErrors({})}}
                          style={{flex:1,padding:"12px 16px",cursor:"pointer",textAlign:"left",
                            ...
                            background:ciChoice===opt.val?"rgba(201,168,76,.12)":"rgba(255,255,255,.02)",border:"none",
              ...
              <div style={{padding:"14px 18px",background:"rgba(82,192,122,.06)",borderBottom:"1px solid rgba(82,192,122,.15)"}}>
                ...
                <p style={{fontSize:11,color:"#8A7E6A",margin:"4px 0 0"}}>
                  {isExtended
                    ...
                    : isEarly
                      ? `Early checkout · ${unusedNights} unused night${unusedNights>1?"s":""} · 10% fee = ${fmtINR(totalDeduction)}`
                      : checkoutDue>0
                        ? `${fmtINR(checkoutDue)} due — collect before checking out`
