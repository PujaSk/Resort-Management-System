...
// Used by both admin (/admin/profile) and staff (/staff/profile)
    ...
    <div style={{ display:"flex", gap:8, alignItems:"flex-end" }}>
      ...
      <button onClick={onResend} disabled={!canResend}
        style={{ padding:"10px 14px", borderRadius:9, fontSize:12, fontWeight:600, cursor: canResend?"pointer":"not-allowed",
          ...
          color: canResend?"#C9A84C":"#6B6054", whiteSpace:"nowrap", marginBottom:0 }}>
    ...
    <div>
      ...
      <div className="page-hd">
        ...
        <div style={{ width:52, height:52, borderRadius:14, display:"flex", alignItems:"center",
          ...
          background:"rgba(201,168,76,.12)", border:"2px solid rgba(201,168,76,.25)" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, maxWidth:900 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          <SectionCard title="Personal Info" icon= {<UserIcon size={20}/>}>
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              {/* Read-only designation for staff */}
              {isStaff && (
                <Field label="Designation" value={user?.designation || "—"} disabled/>
              )}
              {isStaff && (
                <Field label="Shift" value={user?.shift || "—"} disabled/>
              )}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <SectionCard title="Change Password" icon={<LockIcon/>}>
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <p style={{ fontSize:12, color:"#6B6054", marginBottom:2 }}>
                Know your current password? Change it directly.
              </p>
                ...
                {pwForm.newPassword && (
                  <div style={{ marginTop:6, display:"flex", gap:3 }}>
                    ...
                    <span style={{ fontSize:10, color:"#6B6054", marginLeft:6, whiteSpace:"nowrap" }}>
                      {[pwForm.newPassword.length>=6,/[A-Z]/.test(pwForm.newPassword),/[0-9]/.test(pwForm.newPassword),/[^A-Za-z0-9]/.test(pwForm.newPassword)].filter(Boolean).length < 2
                        ? "Weak" : [pwForm.newPassword.length>=6,/[A-Z]/.test(pwForm.newPassword),/[0-9]/.test(pwForm.newPassword),/[^A-Za-z0-9]/.test(pwForm.newPassword)].filter(Boolean).length < 4
                        ? "Good" : "Strong"}
                    </span>
                  </div>
                )}
                ...
                <div style={{ position:"relative" }}>
                  <input type={showPw.con ? "text" : "password"}
                    ...
                    style={{ width:"100%", boxSizing:"border-box", padding:"10px 40px 10px 13px",
                      ...
                      color: pwForm.confirmPassword
                        ? pwForm.confirmPassword === pwForm.newPassword ? "#52C07A" : "#E05252"
                        : "#F5ECD7",
                      background:"rgba(255,255,255,.04)", border: `1px solid ${
                        pwForm.confirmPassword
                          ? pwForm.confirmPassword === pwForm.newPassword ? "rgba(82,192,122,.3)" : "rgba(224,82,82,.3)"
                          : "rgba(255,255,255,.1)"}` }}
                    onFocus={e => e.target.style.border="1px solid rgba(201,168,76,.4)"}
                    onBlur={e  => e.target.style.border=`1px solid ${
                      e.target.value
                        ? e.target.value === document.querySelector('input[placeholder="Repeat new password"]')?.previousSibling?.value ? "rgba(82,192,122,.3)" : "rgba(224,82,82,.3)"
                        : "rgba(255,255,255,.1)"}`}/>
              ...
              {otpStep === "idle" && (
                <>
                  <p style={{ fontSize:12, color:"#6B6054", lineHeight:1.7 }}>
                    Don't remember your current password? We'll send a 6-digit OTP to<br/>
                    <strong style={{ color:"#C9A84C" }}>{user?.email}</strong>
                  </p>
                  <button onClick={sendOtp}
                    style={{ padding:"10px 18px", borderRadius:9, fontWeight:700, fontSize:13,
                      cursor:"pointer", border:"1px solid rgba(201,168,76,.3)", alignItems:"center", gap:4,
                      background:"rgba(201,168,76,.08)", color:"#C9A84C", display: "inline-flex", 
                ...
                <>
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    <button onClick={() => setOtpStep("idle")}
                      style={{ flex:1, padding:"10px", borderRadius:9, fontSize:12, fontWeight:600, cursor:"pointer",
                        border:"1px solid rgba(255,255,255,.1)", background:"transparent", color:"#6B6054" }}>
                      Cancel
                    </button>
                    <button onClick={verifyOtp}
                      style={{ flex:2, padding:"10px", borderRadius:9, fontSize:13, fontWeight:700, cursor:"pointer",
                        background:"linear-gradient(135deg,#C9A84C,#E0C06A)", color:"#0E0C09", border:"none" }}>
                      Verify OTP
                    </button>
                  </div>
                </>
