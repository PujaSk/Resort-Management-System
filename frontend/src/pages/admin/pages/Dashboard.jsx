    ...
    <div className={`stat-card anim-up d${delay}`} style={{ position:"relative", overflow:"hidden" }}>
      ...
      <div className="flex items-start justify-between mt-1">
        <div style={{ minWidth:0 }}>
          ...
          <p className="font-display font-bold" style={{ color:accent, fontSize:"clamp(24px,3.5vw,36px)" }}>{value}</p>
        ...
        <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"
          style={{ background:`${accent}15`,border:`1px solid ${accent}22`,color:accent }}>
          <Icon/>
        </div>
      </div>
    </div>
...
function Chip({ label, color }) {
  return <span style={{ fontSize:10,fontWeight:600,padding:"2px 8px",borderRadius:20,background:`${color}15`,color,border:`1px solid ${color}22` }}>{label}</span>
}
...
export default function Dashboard() {
  ...
  const bannerStats = [
    ...
    { Icon:Icons.Star,      label:"Avg Rating",        val:avgRating,      col:C.gold  },
  ]

  return (
    <>
      <style>{`
        /* Stat cards: 2 col on mobile → 3 col on md → 6 col on xl */
        .dash-stats {
          display: grid;
        ...
        @media (min-width: 1280px) { .dash-stats { grid-template-columns: repeat(6, 1fr); } }

        /* Body layout: 1 col on mobile, 2 col on xl */
        .dash-body {
          display: grid;
        ...
        @media (min-width: 1280px) {
          .dash-body { grid-template-columns: 2fr 1fr; }
          /* Right column sticks below navbar and stops at its own natural end.
             Left column (with extra Recent Guests card) scrolls freely past it. */
          .dash-right {
            position: sticky;
            top: 4.5rem;
            align-self: start;
        ...
        /* Banner quick-stat chips wrap on mobile */
        .banner-stats { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }

        /* 2-col booking summary mini grid */
        .booking-mini-grid {
          display: grid;
          grid-template-columns: 1fr 1fr;
          gap: 12px;
        }
        @media (max-width: 400px) { .booking-mini-grid { grid-template-columns: 1fr; } }
      `}</style>

      <div>
        {/* ── Greeting banner ── */}
        <div className="anim-up mb-7" style={{
          padding:"clamp(14px,2.5vw,20px) clamp(16px,3vw,24px)",
          borderRadius:14,
          background:"linear-gradient(120deg,rgba(201,168,76,.08) 0%,rgba(255,255,255,.02) 100%)",
          border:"1px solid rgba(201,168,76,.14)",
          display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12,
        }}>
          <div>
            <h1 className="font-display font-bold text-cream" style={{ fontSize:"clamp(20px,3vw,28px)" }}>
              Good {greeting()}, Admin <span className="text-gold">✦</span>
            </h1>
            <p className="text-resort-muted text-sm mt-1.5">
              {new Date().toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"long",year:"numeric"})} · Resort overview
            </p>
          </div>
          <div className="banner-stats">
            {bannerStats.map((t,i)=>(
              <div key={i} style={{ padding:"9px 14px",borderRadius:10,textAlign:"center",background:`${t.col}10`,border:`1px solid ${t.col}22` }}>
                ...
                <div style={{ fontSize:"clamp(15px,2.5vw,18px)",fontWeight:800,color:t.col,lineHeight:1.1 }}>{t.val}</div>
        ...
        <div className="dash-body">
          ...
          {/* LEFT — scrolls naturally with page */}
          <div className="space-y-5">
            ...
            <div className="card-p anim-up d4">
              ...
              <div className="booking-mini-grid mt-4 pt-4" style={{ borderTop:"1px solid rgba(255,255,255,.05)" }}>
                {[
                  ...
                  <div key={i} style={{ padding:"9px 12px",borderRadius:10,background:`${m.col}10`,border:`1px solid ${m.col}1A` }}>
                    <p style={{ fontSize:"clamp(13px,2vw,16px)",fontWeight:800,color:m.col }}>{m.val}</p>
                  ...
                  <DivRow key={c._id||i}>
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      {c.isEmailVerified!=null && (
                        <div className="mt-1"><Chip label={c.isEmailVerified?"Verified":"Unverified"} color={c.isEmailVerified?C.green:C.amber}/></div>
                      )}
                    </div>
                  </DivRow>
                ))
          ...
          {/* RIGHT — sticky on desktop, stacks below on mobile */}
          <div className="dash-right space-y-5">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} style={{ padding:"11px 14px",borderRadius:10,background:`${m.col}10`,border:`1px solid ${m.col}1A`,display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                    ...
                    <span style={{ fontSize:"clamp(14px,2vw,16px)",fontWeight:800,color:m.col }}>{m.val}</span>
                  </div>
                ))}
              </div>
            </div>
