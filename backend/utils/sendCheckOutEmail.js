...
const sendCheckOutEmail = async ({
  ...
  isHallBk = false, amountCollected = 0,
  paymentMethod = null, upiId = null, cardNumber = null,
  isEarlyCheckout = false, earlyCheckoutDetails = null,
  isExtendedStay = false, extendedStayDetails = null,
    ...
    const earlyBlock = isEarlyCheckout && earlyCheckoutDetails ? `
      <div style="background:#fef3c7;border-left:4px solid #f59e0b;padding:15px 20px;border-radius:0 8px 8px 0;margin-bottom:24px;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr><td style="color:#666;font-size:12px;padding:3px 0;">Nights Stayed</td><td style="color:#222;font-size:12px;font-weight:600;text-align:right;">${earlyCheckoutDetails.stayedNights} night${earlyCheckoutDetails.stayedNights !== 1 ? "s" : ""}</td></tr>
          <tr><td style="color:#666;font-size:12px;padding:3px 0;">Unused Nights</td><td style="color:#222;font-size:12px;font-weight:600;text-align:right;">${earlyCheckoutDetails.unusedNights} night${earlyCheckoutDetails.unusedNights !== 1 ? "s" : ""}</td></tr>
          <tr><td style="color:#666;font-size:12px;padding:3px 0;">Early Checkout Fee (10%/night)</td><td style="color:#d97706;font-size:12px;font-weight:700;text-align:right;">₹${fmt(earlyCheckoutDetails.totalDeduction)}</td></tr>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;border-radius:10px;overflow:hidden;border:1px solid #e8e0d0;">
        <tr>
          <td style="background:linear-gradient(135deg,#1a1208,#2d2010);padding:24px 30px;text-align:center;">
            ...
            <p style="margin:0 0 18px;color:#C9A84C;font-size:13px;line-height:1.6;">Your feedback helps us serve every guest better.<br>It takes less than a minute — and means the world to us.</p>
            ...
            <p style="margin:14px 0 0;color:#6b5c3e;font-size:11px;">Visit My Bookings → find this booking → click Details → Rate Your Stay</p>
          </td>
        </tr>
      </table>
    ` : "";
          ...
          <table width="620" cellpadding="0" cellspacing="0"
            ...
            <tr>
              <td style="background:linear-gradient(135deg,#1a1208 0%,#2d2010 50%,#1a1208 100%);padding:40px 40px 30px;text-align:center;">
                ...
                <h1 style="margin:0;font-size:28px;color:#f5edd8;font-weight:700;letter-spacing:1px;">${isHallBk ? "Event Completed" : "Thank You for Staying With Us"}</h1>
                <p style="margin:10px 0 0;color:#C9A84C;font-size:14px;">✦ &nbsp;${isHallBk ? "Your event is complete" : "We hope to see you again soon"}&nbsp; ✦</p>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="padding:35px 40px;">
                <p style="color:#333;font-size:15px;line-height:1.7;margin:0 0 25px;">
                  ...
                  ${isHallBk ? `Thank you for choosing <strong>Royal Palace Resort</strong> for your event. Your check-out has been processed and your booking is now complete.` : `Your check-out from <strong>Royal Palace Resort</strong> has been processed. We truly enjoyed hosting you and hope your stay exceeded expectations.`}
                </p>
                <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:25px;border-radius:8px;overflow:hidden;border:1px solid #e8e0d0;">
                  <tr><td colspan="2" style="background:#f9f5ee;padding:12px 16px;font-size:12px;letter-spacing:2px;color:#8B6914;font-weight:700;text-transform:uppercase;border-bottom:1px solid #e8e0d0;">${isHallBk ? "🏛 Event Summary" : "🛏 Stay Summary"}</td></tr>
                  ${isHallBk ? `
                  <tr><td style="padding:12px 16px;color:#888;font-size:13px;width:45%;border-bottom:1px solid #f0ebe2;">Venue</td><td style="padding:12px 16px;color:#222;font-size:13px;font-weight:700;border-bottom:1px solid #f0ebe2;">${roomType?.type_name || "—"}</td></tr>
                  <tr style="background:#fdfaf5;"><td style="padding:12px 16px;color:#888;font-size:13px;">Event Date</td><td style="padding:12px 16px;color:#222;font-size:13px;font-weight:700;">${fmtDate(booking.checkInDateTime)}</td></tr>
                  ` : `
                  <tr><td style="padding:12px 16px;color:#888;font-size:13px;width:45%;border-bottom:1px solid #f0ebe2;">Room Type</td><td style="padding:12px 16px;color:#222;font-size:13px;font-weight:700;border-bottom:1px solid #f0ebe2;">${roomType?.type_name || "—"}</td></tr>
                  ...
                  <tr><td style="padding:12px 16px;color:#888;font-size:13px;border-bottom:1px solid #f0ebe2;">Check-In Date</td><td style="padding:12px 16px;color:#222;font-size:13px;font-weight:700;border-bottom:1px solid #f0ebe2;">${fmtDate(booking.checkInDateTime)}</td></tr>
                  <tr style="background:#fdfaf5;"><td style="padding:12px 16px;color:#888;font-size:13px;border-bottom:1px solid #f0ebe2;">Check-Out Date</td><td style="padding:12px 16px;color:#222;font-size:13px;font-weight:700;border-bottom:1px solid #f0ebe2;">${fmtDate(new Date())}</td></tr>
                  `}
                  <tr><td style="padding:12px 16px;color:#888;font-size:13px;">Total Amount Paid</td><td style="padding:12px 16px;color:#2e7d32;font-size:14px;font-weight:700;">₹${fmt(totalPaid)}</td></tr>
                </table>
                ${earlyBlock}
                ${receiptRows.length > 0 ? `
                <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:25px;border-radius:8px;overflow:hidden;border:1px solid #e8e0d0;">
                  <tr><td colspan="3" style="background:#f9f5ee;padding:12px 16px;font-size:12px;letter-spacing:2px;color:#8B6914;font-weight:700;text-transform:uppercase;border-bottom:1px solid #e8e0d0;">💰 Full Payment Receipt</td></tr>
                ...
                <div style="background:linear-gradient(135deg,#fdf8ee,#f5edd8);border:1px solid #e8d5a3;border-radius:10px;padding:22px 24px;margin-bottom:25px;text-align:center;">
                  ...
                  <p style="margin:0 0 14px;color:#666;font-size:13px;line-height:1.6;">We'd love to welcome you again. Book your next stay and enjoy our finest hospitality.</p>
